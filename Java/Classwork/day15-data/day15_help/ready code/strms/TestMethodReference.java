...
public class TestMethodReference {
	...
	public static void main(String[] args) {
		...
		// OR
		populateData().stream() //Stream<Product> : un sorted strm
		.sorted(Comparator.comparing
				(p -> p.getManufactureDate()))  //Stream<Product> : sorted  as per date
		.forEach(System.out::println);
		// OR
		populateData().stream().sorted(
				Comparator.comparing(Product::getManufactureDate)).forEach(System.out::println);
